...
struct sockaddr_in their_addr; // connector's address information
    ...
    packData(buffer, "asd", 1, 2);
    
    sendto(sockfd, buffer, sizeof(char)*4, 0, (struct sockaddr *)&their_addr, sizeof(struct sockaddr_in));
...
{
    unsigned char buffer[8];
    recvfrom(sockfd, buffer, 8, 0, NULL, NULL);
